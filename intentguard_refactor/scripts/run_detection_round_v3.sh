...
BASE_RUN_DIR="${BASE_RUN_DIR:-runs/CISR_v3}"
DATA="${DATA:-data/CISR_v3_probe.jsonl}"
DATA_SUMMARY="${DATA_SUMMARY:-data/CISR_v3_probe_summary.json}"
CONFIG="${CONFIG:-intentguard_refactor/configs/cisr_v3_families.json}"
...
MODEL_SPECS="${MODEL_SPECS:-qwen25vl7b|Qwen/Qwen2.5-VL-7B-Instruct|qwen2_5_vl|hf;gemma3_12b|google/gemma-3-12b-it|generic_vlm|modelscope;llava15_7b|llava-hf/llava-1.5-7b-hf|generic_vlm|hf}"
...
if [[ "$RUN_DATA" == "1" ]]; then
  if [[ "$REBUILD_DATA" == "1" || "$FORCE" == "1" || ! -s "$DATA" || ! -s "$DATA_SUMMARY" ]]; then
    echo "[data] Building isolated CISR_v3 paired/multiview probe"
    "$PYTHON_BIN" intentguard_refactor/make_data_v3.py \
      --config "$CONFIG" \
      --out "$DATA" \
...
for spec in "${SPECS[@]}"; do
  ...
  echo "========== [$alias] CISR_v3 model=$model backend=$backend source=$source =========="
  ...
  if [[ "$RUN_ACTIVATIONS" == "1" ]]; then
    if should_run "$activations" || ! activation_cache_is_valid "$activations" "$DATA" "$model" "$backend"; then
      echo "[1/3:$alias] Extracting all layers (raw representation; no anchor forward)"
      "$PYTHON_BIN" src/extract_activations.py \
        --model "$model" \
        "${model_args[@]}" \
        --backend "$backend" \
        --data "$DATA" \
      ...
      echo "[1/3:$alias] Reusing $activations"
    fi
  fi

  if [[ "$RUN_GENERATION" == "1" ]]; then
    if should_run "$generation_results"; then
      echo "[optional:$alias] Generating original responses"
      "$PYTHON_BIN" src/run_probe_generations.py \
        --model "$model" \
        --model-alias "$alias" \
        "${model_args[@]}" \
        --backend "$backend" \
      ...
      echo "[optional:$alias] Reusing $generation_results"
    fi
  fi

  if [[ "$RUN_JUDGE" == "1" ]]; then
    if [[ ! -s "$generation_results" ]]; then
      echo "RUN_JUDGE=1 requires $generation_results; enable RUN_GENERATION or provide the file." >&2
      exit 1
    fi
    if should_run "$judge_results"; then
      echo "[optional:$alias] Judging original responses with ModelScope Gemma3-12B"
      "$PYTHON_BIN" intentguard_refactor/judge_outputs.py \
        --model "$JUDGE_MODEL" \
        "${judge_args[@]}" \
      ...
      echo "[optional:$alias] Reusing $judge_results"
    fi
  fi

  if [[ "$RUN_DETECTOR" == "1" ]]; then
    if [[ ! -s "$activations" ]]; then
      echo "Detector training requires $activations; enable RUN_ACTIVATIONS or provide the archive." >&2
      exit 1
    fi
    if ! activation_cache_is_valid "$activations" "$DATA" "$model" "$backend"; then
      echo "Detector training refused an activation archive that does not match the v3 data/model/backend." >&2
      exit 1
    fi
    if [[ "$FORCE" == "1" || "$activations_rebuilt" == "1" || ! -s "$detector_artifact" || ! -s "$detection_summary" || ! -s "$detection_results" ]]; then
      echo "[2/3:$alias] Training raw-rank3 stability-aware detector"
      response_args=()
      if [[ -s "$judge_results" ]]; then
        response_args=(--response-labels "$judge_results")
      fi
      ...
      echo "[2/3:$alias] Reusing $detector_artifact"
    fi
  fi

  echo "[3/3:$alias] Detector: $detector_artifact"
done

echo "CISR_v3 detection round complete under $BASE_RUN_DIR"
